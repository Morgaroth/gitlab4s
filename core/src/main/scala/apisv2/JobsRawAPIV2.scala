...
trait JobsRawAPIV2[F[_]] {
  this: GitlabRestBaseV2[F] =>

  // @see: https://docs.gitlab.com/ee/api/jobs.html#get-a-single-job
  def getJobRaw[T: Decoder](projectId: EntityId, jobId: BigInt): F[Either[GitlabError, T]] = {
  ...
  def cancelJobRaw[T: Decoder](projectId: EntityId, jobId: BigInt): F[Either[GitlabError, T]] = {
  ...
  // @see: https://docs.gitlab.com/ee/api/jobs.html#cancel-a-job
  def retryJobRaw[T: Decoder](projectId: EntityId, jobId: BigInt): F[Either[GitlabError, T]] = {
  ...
  def runJobRaw[T: Decoder](projectId: EntityId, jobId: BigInt, params: TriggerJobParam*): F[Either[GitlabError, T]] = {
    implicit val rId: RequestId = RequestId.newOne("trigger-pipeline-job")
    val req =
      reqGen.post(s"$API/projects/${projectId.toStringId}/jobs/$jobId/play", MJson.write(TriggerJobPayload(params.toVector)), projectId)
    invokeRequest(req).unmarshall[T]
  }
  ...
  def getPipelineBridgesRaw[T: Decoder](
      projectId: EntityId,
      pipelineId: BigInt,
