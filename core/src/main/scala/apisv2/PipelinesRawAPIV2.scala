...
trait PipelinesRawAPIV2[F[_]] {
  ...
  def getProjectPipelinesRaw[T: Decoder](
    ...
    val params = Vector(
      ...
      wrap(source).map("source".eqParam(_)),
    ...
    getAllPaginatedResponse[T](req, "get-pipelines-of-project", paging)
  }

  // @see: https://docs.gitlab.com/ee/api/pipelines.html#get-a-single-pipeline
  def getPipelineRaw[T: Decoder](projectId: EntityId, pipelineId: BigInt): F[Either[GitlabError, T]] = {
  ...
  def getPipelineJobsRaw[T: Decoder](
      projectId: EntityId,
      pipelineId: BigInt,
    ...
    getAllPaginatedResponse[T](req, "get-pipeline-jobs", paging)
  }

  // @see: https://docs.gitlab.com/ee/api/pipelines.html#create-a-new-pipeline
  def triggerPipelineRaw[T: Decoder](
      projectId: EntityId,
      branch: String,
  ...
  def getPipelineVariablesRaw[T: Decoder](
      projectId: EntityId,
      pipelineId: BigInt,
  ): F[Either[GitlabError, Vector[T]]] = {
