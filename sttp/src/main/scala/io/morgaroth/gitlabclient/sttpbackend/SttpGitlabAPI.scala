...
class SttpGitlabAPI(val config: GitlabConfig, apiConfig: GitlabRestAPIConfig)
                   (implicit ex: ExecutionContext) extends GitlabRestAPI[Future] with LazyLogging {
  ...
  private def logRequest[T](request: RequestT[Identity, Either[String, T], Nothing], requestData: GitlabRequest)
  ...
  private def execReq[T](request: RequestT[Identity, Either[String, T], Nothing])
    ...
    request
      ...
      .flatMap { response =>
        ...
        val responseBodyForLog = if (responseContentType.contains("application/json")) response.body.fold(identity, identity) else response.body.fold(identity, _ => s"Response has content type $responseContentType and has $responseLength bytes length")
  ...
  private def createReq(requestData: GitlabRequest) = {
    ...
    requestData.payload.map { rawPayload =>
      requestWithoutPayload.body(rawPayload).contentType("application/json")
    }.getOrElse(requestWithoutPayload)
  }

  override def byteRequest(requestData: GitlabRequest)(implicit requestId: RequestId)
  : EitherT[Future, GitlabError, GitlabResponse[Array[Byte]]] = {
  ...
  override def invokeRequestRaw(requestData: GitlabRequest)(implicit requestId: RequestId)
  : EitherT[Future, GitlabError, GitlabResponse[String]] = {
